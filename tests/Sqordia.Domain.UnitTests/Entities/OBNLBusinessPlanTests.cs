...
public class OBNLBusinessPlanTests
{
    private readonly Fixture _fixture;

    public OBNLBusinessPlanTests()
    {
    ...
    public void Constructor_ShouldGenerateUniqueId()
    {
        ...
        // Assert
        plan1.Id.Should().NotBeEmpty();
        plan2.Id.Should().NotBeEmpty();
    ...
    [Theory]
    [InlineData("Compliant", "Full")]
    [InlineData("Non-Compliant", "None")]
    [InlineData("Pending", "Basic")]
    public void ComplianceStatus_WithValidStatuses_ShouldAcceptAllStatuses(string status, string level)
    {
        // Arrange
        var plan = new OBNLBusinessPlan();
        var complianceStatus = ComplianceStatus.Create(status, level);

        // Act
        plan.ComplianceStatus = complianceStatus;

        // Assert
        plan.ComplianceStatus.Should().Be(complianceStatus);
    }
    ...
    public void CreateOBNLBusinessPlan_WithAllProperties_ShouldCreateCompletePlan()
    {
        ...
        var complianceStatus = ComplianceStatus.Pending;
        var legalStructure = "Non-Profit Corporation";
        var registrationNumber = "123456789";
